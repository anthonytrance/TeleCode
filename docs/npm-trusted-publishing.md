...
This document explains the release automation process used in TelePi and how to reuse it in TeleCode.

## Goal

Publish TeleCode from GitHub Actions without storing a long-lived `NPM_TOKEN` secret.
...
Reuse/adapt the same pattern for TeleCode.

## What TeleCode needs

TeleCode is not yet wired for this flow. To adopt it, plan the following:

1. **Choose the package name**
   - current application package name: `telecodeide`
   - update `package.json` accordingly
   - add `publishConfig.access = public`

2. **Ensure the package is publishable**
   - add a `files` allowlist
   - add a `bin` entry if TeleCode should be globally installed as a CLI
...
## Suggested TeleCode rollout

1. make TeleCode npm-publishable
...
Once TeleCode adopts this setup, document the release flow in:
- `README.md`
- `AGENTS.md`
